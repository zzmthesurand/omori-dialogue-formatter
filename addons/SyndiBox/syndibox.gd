...
export(Array, String, FILE, "*.fnt, *.tres") var SIZE_FONTS # Alternate fonts, [%0] to [%9]
export(Array, String, FILE, "*.fnt, *.tres") var SMALL_FONTS # Alternate fonts, [%0] to [%9]
...
export(String) var ADVANCE_ACTION = "omori_forward" # Action to trigger manual advance
export(String) var ADVANCE_SKIP = "ui_cancel" # Action to trigger manual skip
...
onready var size_fonts : Array
onready var small_fonts: Array
...
onready var ring = AudioStreamRandomPitch.new()
onready var fontSize = 0
...
func _ready(): # Called when ready.
	...
	for f in SIZE_FONTS:
		size_fonts.push_back(load(f))
	for f in SMALL_FONTS:
		small_fonts.push_back(load(f))
	font = def_font
	def_color = COLOR
	color = def_color
	...
	snd_stream.loop = false
	ring.audio_stream = snd_stream
	ring.random_pitch = 1.1
	voice.set_stream(ring)
	voice.volume_db = -14
	add_child(voice)
	...
	if (
		...
		match emph:
			...
			"\\}`":
				fontSize -= 1
		if (emph == "\\{`") || (emph == "\\}`"):
			fontSize = clamp(fontSize, -size_fonts.size(), size_fonts.size())
			if fontSize < 0: 
				font = small_fonts[(-fontSize)-1]
			elif fontSize > 0: 
				font = size_fonts[fontSize-1]
			else: font = def_font
	return string
...
func color_check(string):
	...
	if (
		...
		match emph:
			...
			"\\c[8]`": # RPG OMORI Red?
				color = Color("#fc0d1b")
			"\\c[9]`": # RPG OMORI Lighter Red?
				color = Color("#e10c18")
			"\\c[10]`": # RPG OMORI Lighter Red Again?
				color = Color("#f10c1a")
			"\\c[11]`": # OMORI Light Blue (Places)
				color = Color("#5ae0d8")
			"\\c[12]`": # OMORI Calm Dark Blue
				color = Color("#213d58")
			"\\c[13]`": # OMORI Light Purple
				color = Color("#c263e2")
			"\\c[14]`": # OMORI Calm Dark Blue Again?
				color = Color("#1e3852")
			"\\c[r]`": # Reset
				color = def_color
			"\\<br>`": # New Line
...
func pos_check(string):
	var emph_start = emph.substr(0,4).to_upper()
	if (
		!text_pause && 
		(
			emph_start == "\\SIN" ||
...
func pause_check(string):
	var emph_start = emph.substr(0,emph.length())
	if (
		!text_pause && 
		(
			emph_start == "\\!`" ||
		...
		match emph_start:
			...
			"\\!`":
				#timer.set_wait_time(1)
				manual_text_pause = true
				emit_signal("paused", manual_text_pause)
		string = string.insert(step,char(8203))
		text_pause = true
	return string
...
func _input(event): # Called on input
	...
	if event.is_action_pressed(ADVANCE_ACTION) && (manual_text_pause && visible):
		resumeplus()
		
	if (
		!Engine.editor_hint && event.is_action_pressed(ADVANCE_SKIP) &&
		!AUTO_ADVANCE &&
